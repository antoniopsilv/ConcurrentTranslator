...
class TranslateViewModel: ViewModel() {
    ...
    fun translate(fromLanguage: String, toLanguage: String, text: String) =
        viewModelScope.launch(Dispatchers.IO) {
            TranslateApiClient.service.translate(TranslateRequest(fromLanguage, toLanguage, text)).execute().also { response ->

                Log.v("\n\n\n #### AGORA   $response.code() #####"," $response.code() ")

                if (response.code() == HttpURLConnection.HTTP_OK) {
                    Log.v("\n\n\n ####  Retornou ok no TRANSLATE #####"," ")
                    response.body()?.let { translationResult ->
                        TranslateLiveData.translateResultLiveData.postValue(translationResult)
                    }
                }
                Log.v("\n\n\n ####  NÃO Retornou ok no TRANSLATE #####"," ")
            }
    }
