...
class CurrienciesService: Service() {

    private lateinit var handler: CurrienciesServiceHandler
    private lateinit var serviceLogTag: String

    private inner class CurrienciesServiceHandler(looper: Looper): Handler(looper) {
    ...
    override fun onCreate() {
        ...
        HandlerThread(this.javaClass.name).apply {
            start()
            handler = CurrienciesServiceHandler(looper)
        }
    }
